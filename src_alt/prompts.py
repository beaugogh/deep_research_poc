"""Prompts copied from the main project into src_alt so the alt agents
have no dependency on the top-level `deep_research` package.

Only the prompts used by the agents are included here (copied verbatim).
"""
...
# (Additional prompts omitted for brevity in this file - full set exists in the main repo.
# The above prompts are the ones used in the `src_alt` modules.)
